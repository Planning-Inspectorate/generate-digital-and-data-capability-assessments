...
import {NO_GRADE, ROLE_CONFIG, TEMPLATE_MAX_SKILLS} from "./config.js";
  ...
  for (const role of ROLE_CONFIG) {
    ...
    for (const roleLevel of roleLevels) {
      if (roleLevel.grade === NO_GRADE) {
        ...
        if (roleLevel.skills.length > TEMPLATE_MAX_SKILLS) {
          throw new Error(`${roleLevel.name} has ${roleLevel.skills.length} (max ${TEMPLATE_MAX_SKILLS}) skills`);
        }
